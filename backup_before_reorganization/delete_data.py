...
ALL_TABLES = [
    ...
    'staging_vehicles',    # Delete second (independent)
    'vehicles'            # Delete last (referenced by processed_urls)
]
...
async def delete_table_data(db_handler: DatabaseHandler, table_name: str) -> int:
    ...
    try:
        ...
        result = db_handler.supabase_client.table(table_name).delete().neq("id", 0).execute()
...
async def delete_all_data(db_handler: DatabaseHandler) -> int:
    ...
    print("   2. staging_vehicles (independent)")
    print("   3. vehicles (referenced by processed_urls)")
    print("=" * 50)
